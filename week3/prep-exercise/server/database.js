...
const makeInMemoryDb = () => {
  ...
  return {
    ...
    // getByUsername: (username) => {  // Ensure this method is defined here
...
if (
  ...
  process.argv[1].includes("database.js")
) {
  ...
  // Then
  console.assert(
    persistUser.some.nested === testUser.some.nested,
    "fail to create user from persist db"
  );
  console.assert(
    inMemStoredUser.some.nested === testUser.some.nested,
    "fail to create user from in mem db"
  );
  console.assert(
    persistUser.id !== undefined,
    "persistent db returned user without id"
  );
  console.assert(
    inMemStoredUser.id !== undefined,
    "in mem user returned db without id"
  );
  ...
  // Then
  console.assert(
    foundPersistedUser.name === testUser.name,
    "retrieving user from persistent db failed"
  );
  console.assert(
    foundInMemUser.name === testUser.name,
    "retrieving user from inMem db failed"
  );
  console.assert(
    notFoundPersistedUser === undefined,
    "persistent db returned a user from an unknown ID"
  );
  console.assert(
    notFoundMemUser === undefined,
    "in mem db returned a user from an unknown ID"
  );
